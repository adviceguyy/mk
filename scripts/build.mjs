...
function extractAssets(timestamp) {
  ...
  extractFromBundle(bundles.web, "web");
...
function generateWebIndex(timestamp, baseUrl) {
  const webBundleUrl = `${baseUrl}/${timestamp}/_expo/static/js/web/bundle.js`;

  // Determine which favicon to use - prefer .ico, fallback to .png
  const faviconIcoPath = path.resolve(process.cwd(), "assets", "favicon.ico");
  const faviconLink = fs.existsSync(faviconIcoPath)
...
<html lang="en">
  ...
  console.log(`Generated index.html pointing to ${webBundleUrl}`);
}
...
async function main() {
  ...
  console.log("Updating manifests and generating web index...");
  updateManifests(manifests, timestamp, baseUrl, assetsByHash);
  generateWebIndex(timestamp, baseUrl);

  // Copy static assets to dist for production serving
  const assetsDir = path.resolve(process.cwd(), "assets");
  const distAssetsDir = path.resolve(process.cwd(), "dist", "assets");
  const clientAssetsDir = path.resolve(process.cwd(), "client", "assets");
  const distClientAssetsDir = path.resolve(process.cwd(), "dist", "client", "assets");
  ...
  // Copy favicon to dist root for web
  const faviconIcoSrc = path.resolve(process.cwd(), "assets", "favicon.ico");
  const faviconPngSrc = path.resolve(process.cwd(), "assets", "favicon.png");
  const faviconDest = path.resolve(process.cwd(), "dist", "favicon.ico");
  const faviconPngDest = path.resolve(process.cwd(), "dist", "favicon.png");

  if (fs.existsSync(faviconIcoSrc)) {
    fs.copyFileSync(faviconIcoSrc, faviconDest);
    console.log("Copied favicon.ico to dist");
  } else if (fs.existsSync(faviconPngSrc)) {
    // Copy PNG as fallback and also to dist for reference
    fs.copyFileSync(faviconPngSrc, faviconPngDest);
    console.log("Copied favicon.png to dist (convert to .ico for best browser support)");
  }
  
  // Copy client/assets for Expo web unstable_path resolution
  if (fs.existsSync(clientAssetsDir)) {
    console.log("Copying client assets to dist...");
    fs.mkdirSync(path.dirname(distClientAssetsDir), { recursive: true });
    fs.cpSync(clientAssetsDir, distClientAssetsDir, { recursive: true });
  }
  
  // Fix permissions for web server access
  try {
    if (fs.existsSync(assetsDir)) execSync(`find "${assetsDir}" -type f -exec chmod 644 {} +`);
    if (fs.existsSync(distAssetsDir)) execSync(`find "${distAssetsDir}" -type f -exec chmod 644 {} +`);
    if (fs.existsSync(clientAssetsDir)) execSync(`find "${clientAssetsDir}" -type f -exec chmod 644 {} +`);
    if (fs.existsSync(distClientAssetsDir)) execSync(`find "${distClientAssetsDir}" -type f -exec chmod 644 {} +`);
    console.log("Fixed asset file permissions");
  } catch (e) {
    console.warn("Could not fix permissions (may not be critical):", e.message);
  }
